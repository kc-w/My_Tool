...
LightButton::LightButton(QWidget *parent) : QFrame(parent)
{
    text = "";
...
bool LightButton::eventFilter(QObject *target, QEvent *event)
{
...
//                //通过该控件对象名查找到该控件
//                LightButton *frame = ui->findChild<LightButton*>(watched->objectName());
            }
        }


//        //鼠标拖动监听,可以对控件进行拖动
//        if (mouseEvent->type() == QEvent::MouseMove && pressed) {
...
//        }
//        if (mouseEvent->type() == QEvent::MouseButtonRelease && pressed) {
...
//        setMask(QRegion(0,0,side,side,QRegion::Ellipse));
