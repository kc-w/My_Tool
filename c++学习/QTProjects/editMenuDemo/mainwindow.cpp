    ...
    , ui(new Ui::MainWindow)
{
    ...
    QAction * action_Open = editMenu ->addAction(QIcon(":/new/prefix1/ioc.png"), tr("打开文件(&O)"));// 添加打开菜单
    ...
    //显示临时消息，显示2000毫秒即2秒
    ui ->statusBar ->showMessage( tr("欢迎使用多文档编辑器"),2000);//创建标签，设置标签样式并显示信息，然后将其以永久部件的形式添加到状态栏
    ...
    editMenu ->addAction(action); //将action的getText()信号和这里的setText()槽进行关联
...
void MainWindow::on_action_5_triggered()
{
    ...
    //使用QMdiArea类的addSubWindow()函数创建子窗口以文本编辑器为中心部件
